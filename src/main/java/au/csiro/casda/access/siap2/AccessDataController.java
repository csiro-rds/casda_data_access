...
/**
 * Controller for the SIAP2 Access Data 'async' and 'sync' requests.
 * <p>
 * Copyright 2015, CSIRO Australia. All rights reserved.
 */
@Api(value = "SIAP2 Access Data", description = "ASYNC end-points")
@RestController
/*
...
{
    private static final int SIAP_SYNC_REQUEST_GRACE_PERIOD_MINUTES = 10;
    ...
    private final long pollTimeForSiapSyncJobsMillis;

    private final long timeoutForSiapSyncJobsMillis;

    private final long sizeLimitForSiapSyncJobsKb;
     ...
     * @param pollTimeForSiapSyncJobsMillis
     *            how often to poll for SIAP sync jobs to see if the job has completed
     * @param timeoutForSiapSyncJobsMillis
     *            maximum timeout for SIAP sync jobs to complete
     * @param sizeLimitForSiapSyncJobsKb
     *            size limit for SIAP sync download requests
     */
    @Autowired
    public AccessDataController(HealthEndpoint healthEndpoint, SystemStatus systemStatus,
            ...
            @Value("${siap.sync.job.poll.time.millis}") long pollTimeForSiapSyncJobsMillis,
            @Value("${siap.sync.job.timeout.millis}") long timeoutForSiapSyncJobsMillis,
            @Value("${siap.sync.job.size.limit.kb}") long sizeLimitForSiapSyncJobsKb)
    {
        ...
        this.pollTimeForSiapSyncJobsMillis = pollTimeForSiapSyncJobsMillis;
        this.timeoutForSiapSyncJobsMillis = timeoutForSiapSyncJobsMillis;
        this.sizeLimitForSiapSyncJobsKb = sizeLimitForSiapSyncJobsKb;
    }
    ...
    /**
     * Returns an XML response that shows the SIAP async job list. The list will always be empty because we do not
     * support job owners.
     * 
    ...
    public @ResponseBody ResponseEntity<String> getJobList()
    {
        ...
        try
        {
            return new ResponseEntity<String>(serializer.getJobList(new JobList("CASDA SIAP ASYNC"), true), httpHeaders,
                    HttpStatus.OK);
        }
    ...
    /**
     * Web end point for performing a sync access data request according to the SIAP Access Data specification.
     * 
     ...
     * 
     * Filtering parameters as defined in the SIAP Access Data specification are currently not supported for SIAP sync
     * requests.
     * 
     * @param request
     *            the request
     ...
     */
    @RequestMapping(method = { RequestMethod.POST, RequestMethod.GET }, value = ACCESS_DATA_SYNC_BASE_PATH)
    public void syncDownloadDataProduct(HttpServletRequest request, HttpServletResponse response)
            throws ResourceNotFoundException, BadRequestException, InterruptedException
    {
        ...
        DataAccessJob dataAccessJob = accessJobManager.createDataAccessJob(jobDto, sizeLimitForSiapSyncJobsKb, true);
        ...
        try
        {
            ...
            while (true)
            {
                if (AccessJobManager.UWS_JOB_FINISHED_EXECUTION_PHASES
                        .contains(accessJobManager.getJobStatus(dataAccessJob.getRequestId()).getPhase()))
                {
                    if (ExecutionPhase.COMPLETED == accessJobManager.getJobStatus(dataAccessJob.getRequestId())
                            .getPhase())
                    {
                ...
                else if (start + timeoutForSiapSyncJobsMillis < System.currentTimeMillis())
                {
                    try
                    {
                    ...
                    {
                        ExecutionPhase phase = accessJobManager.getJobStatus(dataAccessJob.getRequestId()).getPhase();
                        if (!(AccessJobManager.UWS_JOB_FINISHED_EXECUTION_PHASES.contains(phase)))
                        {
                            throw new RuntimeException(
                                    "Could not cancel job created to refresh SIAP SYNC files in request after timeout. "
                ...
                Thread.sleep(pollTimeForSiapSyncJobsMillis);
            }
        }
        catch (IOException e)
        {
        ...
        catch (TimeoutException | ScheduleJobException e)
        {
            logger.info(
        ...
        else
        {
            ...
            dataAccessService.markRequestCompleted(dataAccessJob.getRequestId(),
                    DateTime.now(DateTimeZone.UTC).plusMinutes(SIAP_SYNC_REQUEST_GRACE_PERIOD_MINUTES));
        }

        logger.info("{}", CasdaDataAccessEvents.E040.messageBuilder().add(dataAccessJob.getRequestId())
                .add(path.toFile().getName()));

        dataAccessService.downloadFile(dataAccessJob, path.toFile().getName(), response, skipCacheCheck);
    }
    ...
    public @ResponseBody ResponseEntity<String> createJob(HttpServletRequest request)
    {
        ...
        jobDetails.setDownloadMode(CasdaDownloadMode.SIAP_ASYNC);
        jobDetails.setDownloadFormat(null); // Used for catalogues only
    ...
    @RequestMapping(value = ACCESS_DATA_ASYNC_BASE_PATH + "/{jobId}/results", method = RequestMethod.GET,
            produces = { MediaType.APPLICATION_XML_VALUE })
    public @ResponseBody ResponseEntity<String> getAsyncJobResults(@PathVariable() String jobId)
            throws ResourceNotFoundException
    {
    ...
    private UWSJob getNonExpiredUWSJobForJobId(String jobId) throws ResourceNotFoundException
    {
        ...
        return accessJobManager.getJobStatus(jobId);
    }
            ...
            throws ResourceNotFoundException, ResourceIllegalStateException, ScheduleJobException
    {
        ...
        ExecutionPhase phase = accessJobManager.getJobStatus(jobId).getPhase();
    ...
    private List<String> validateIdParams(String[] authenticatedIdTokens, String userId, String loginSystem)
    {
        ...
        for (String authenticatedIdToken : authenticatedIdTokens)
        {
            ...
            try
            {
                ...
                else if (!EnumSet.of(DataAccessProductType.cube, DataAccessProductType.visibility)
