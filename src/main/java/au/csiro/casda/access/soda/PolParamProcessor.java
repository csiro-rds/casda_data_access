...
public class PolParamProcessor implements CutoutParamProcessor
{
    ...
    /**
     * Extract the combinations of POL parameters from an array of pol states. The combinations match the ranges 
     * generated by getPolRanges
     * 
     * @param polParams
     *            An array of Stokes polarisation string (I, Q etc)
     * @return A list of parameter combinations, will not be null.
     */
    public static List<String> getPolParams(String[] polParams)
    {
        List<String> polParamList = new ArrayList<>();
        if (ArrayUtils.isEmpty(polParams))
        {
            return polParamList;
        }
        ...
        for (PolarizationStateType pol : PolarizationStateType.values())
        {
            String paramValue = "POL=" + pol.toString();
            if (polStates.contains(pol))
            {
                if (currParam == null)
                {
                    currParam = paramValue;
                }
                else
                {
                    currParam += "," + paramValue;
            ...
            else if (currParam != null)
            {
                polParamList.add(currParam);
                currParam = null;
            }
        }
        if (currParam != null)
