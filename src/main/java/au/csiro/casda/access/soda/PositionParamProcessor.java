...
public class PositionParamProcessor implements CutoutParamProcessor
{
    ...
    /**
     * @param testParam the param to validate
     * @param errorList the list of errors to popluate
     */
    protected void validatePolygon(String testParam, List<String> errorList)
    {
        String[] parts = testParam.replace("POLYGON ", "").split(" ");
     ...
     * @param errorList the list of errors to popluate
     */
    protected void validateCircle(String testParam, List<String> errorList)
    {
        String[] parts = testParam.replace("CIRCLE ", "").split(" ");
        String error = verifyLongitude(parts[0], false);
        if (StringUtils.isNotBlank(error))
        {
            errorList.add(error);
            return;
        }
        error = verifyLatitude(parts[1], false);
        if (StringUtils.isNotBlank(error))
        {
            errorList.add(error);
            return;
